...
| 품질 관리는 | 자동 테스트 62개 통과. 클래스 파싱(공식 약어 Cr/In/Pa/PS/RS/Sc 포함), NEU-CLS 입력 계약, 분할 비율·결정론, 업로드 검증, 손상 이미지 처리, 예측 계약(온도 보정 포함), 지표 계약, 근접 중복 검토, ZIP 경로 탈출, 벤치마크 장치·반복 입력 계약을 코드로 고정 |
...
python -m defect_cls.evaluate --checkpoint data/artifacts/baseline/checkpoint.pt
python -m defect_cls.evaluate --checkpoint data/artifacts/augmented/checkpoint.pt
python -m defect_cls.benchmark --checkpoint data/artifacts/baseline/checkpoint.pt --devices cpu,cuda
